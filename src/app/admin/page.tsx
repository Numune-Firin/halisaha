  ...
  return (
    ...
    >
      {(pending.length > 0 || invites.length > 0) && (
        <Link href="/admin/members" className="card card-pad card-link flex flex-col gap-1">
          <span className="text-sm font-semibold text-frost-100">
            Üyeler ve davetler
            {pending.length > 0 && (
              <span className="badge badge-vip ml-2">{pending.length} onay bekliyor</span>
            )}
          </span>
          <span className="hint">
            {pending.length > 0
              ? `${pending.length} kişi onayını bekliyor.`
              : 'Bekleyen üye yok.'}{' '}
            {invites.length > 0 ? `${invites.length} davet kabul edilmedi.` : 'Açık davet yok.'}
          </span>
        </Link>
      )}
