...
export async function createSeason(formData: FormData) {
  return runAction('Sezon açıldı', async () => {
    await requireAdmin();
...
export async function updateSeason(seasonId: string, formData: FormData) {
  return runAction('Sezon güncellendi', async () => {
    await requireAdmin();
    const { name, startsOn, endsOn } = readSeasonForm(formData);
...
export async function activateSeason(seasonId: string) {
  return runAction('Sezon aktif edildi', async () => {
    await requireAdmin();
    const supabase = await createServerSupabase();
...
export async function closeSeason(seasonId: string) {
  return runAction('Sezon kapatıldı', async () => {
    await requireAdmin();
    const supabase = await createServerSupabase();
