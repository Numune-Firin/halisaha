...
export async function approveMember(playerId: string) {
  return runAction('Üye onaylandı', async () => {
    await requireAdmin();
...
export async function openPoll(formData: FormData) {
  return runAction('Anket açıldı', async () => {
    await requireAdmin();
    const supabase = await createServerSupabase();
...
export async function addVip(matchId: string, playerId: string) {
  return runAction('VIP olarak listeye eklendi', async () => {
    await requireAdmin();
    const supabase = await createServerSupabase();
...
export async function markPriority(matchId: string, playerId: string) {
  return runAction('Öncelikli olarak işaretlendi', async () => {
    await requireAdmin();
    const supabase = await createServerSupabase();
...
export async function addAdjustment(playerId: string, seconds: number, reason: string) {
  return runAction('Ceza/ödül işlendi', async () => {
    await requireAdmin();
...
/**
 * E-posta ile davet. Davetli kisi Google ile girdigi anda onay beklemeden
 * aktif uye olur; isaretlendiyse yonetici olarak acilir. Kural veritabaninda,
 * yeni kullanici tetikleyicisinde durur.
 */
export async function inviteMember(formData: FormData) {
  return runAction('Davet kaydedildi', async () => {
    await requireAdmin();
    ...
    const email = String(formData.get('email') ?? '')
      .trim()
      .toLowerCase();
    if (!email.includes('@') || email.length < 5) throw new Error('Geçerli bir e-posta gir');

    const supabase = await createServerSupabase();
    const { error } = await supabase.from('member_invites').upsert(
      {
        email,
        make_admin: formData.get('makeAdmin') === 'on',
...
export async function deleteInvite(email: string) {
  return runAction('Davet silindi', async () => {
    await requireAdmin();

    const supabase = await createServerSupabase();
    const { error } = await supabase.from('member_invites').delete().eq('email', email);
    if (error) throw new Error(error.message);
